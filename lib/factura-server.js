...
// URL pública FIJA de Soporte Móvil (producción, fallback canónico).
export function appPublicUrl() {
  return (process.env.NEXT_PUBLIC_APP_URL || 'https://soportemovil.netlify.app').replace(/\/$/, '');
}

// Hosts de producción válidos para el OAuth. Se usa el origin real desde el que
// entra el usuario (así vuelve al MISMO dominio y no pierde la sesión), pero se
// bloquean los subdominios de deploy preview/branch (que Facturá rechazaría).
const HOSTS_PROD = [
  'soportemovil.com.ar',
  'www.soportemovil.com.ar',
  'soportemovil.netlify.app',
];
export function baseRedirect(origin) {
  try {
    const u = new URL(origin);
    if (HOSTS_PROD.includes(u.host)) return `${u.protocol}//${u.host}`;
  } catch {
    /* origin inválido → canónico */
  }
  return appPublicUrl();
}
